...
export class RAGService {
  ...
  async ingestMediaData() {
    return this.indexingService.indexMediaData();
  }
  ...
  async retrieveRelevantDocuments(
    ...
    try {
      ...
      const results = await prisma.$queryRaw(Prisma.sql`
          SELECT id, "chunkKey", "sourceType", "sourceId", "sourceLabel", content, metadata, embedding::text, "isDeleted", "deletedAt", "createdAt", "updatedAt", 1 - (embedding <=> CAST(${vectorLiteral} AS vector(2048))) as similarity
    ...
    try {
      ...
      if (asJson) {
        try {
          ...
          // Fallback to raw answer if JSON parse fails
        }
      }
